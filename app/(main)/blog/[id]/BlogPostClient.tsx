...
function getInitials(nameOrEmail?: string | null) {
  const base = String(nameOrEmail || "U").trim();
  if (!base) return "U";
...
export default function BlogPostClient({
  ...
  const showAuthorImage = !!post.author?.image && authorImg !== "/placeholder-blog.svg";
          ...
          >
            <div className="max-w-none space-y-8 bg-white lg:border lg:border-slate-100 lg:rounded-2xl lg:p-8 xl:p-10 2xl:p-12 lg:shadow-sm">
              ...
              <div className="flex flex-wrap items-center gap-3 text-sm text-slate-500">
                ...
                {/* ✅ NEW: Author display (image + name) */}
                <span className="text-slate-300">•</span>
                <div className="flex items-center gap-2 min-w-0">
                  <div className="h-8 w-8 rounded-full overflow-hidden border border-slate-200 bg-slate-100 flex items-center justify-center text-slate-700 text-xs font-semibold shrink-0">
                    {showAuthorImage ? (
                      <img
                        src={authorImg}
                        alt={authorName}
